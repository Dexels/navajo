...
public class ConnectionBrokerManager extends Object implements ResourceManager, ConnectionBrokerManagerMBean {
     ...
     *            , log file name
     * @param rfrsh
     *            , refresh rate; if set to 0, set non-broker mode
     * @param ac
     *            , autocommit flag
     * @param forcecreation
     *            , if set to true don't search for exact or similar brokers,
     *            just create new one.
     * 
     * @throws ClassNotFoundException
     */
    public final void put(final String dsrc, final String drv, final String url, final String usr, final String pwd,
            final int minconn, final int maxconn, final String lfile, final double rfrsh, final Boolean ac,
            final boolean forcecreation, final String type) throws ClassNotFoundException {
        ...
        synchronized (semaphore) {
            ...
            if (!forcecreation) {
                ...
                if (broker != null) {
                    ...
                    if (this.debug) {
                        logger.info(this.getClass() + ": already have a broker for data source '" + dsrc
                                + "', user name '" + usr + "'");
                    }
                    return;
                }
            ...
            if (similar != null) {
                if (this.debug) {
                    logger.info(this.getClass() + ": have a similar broker for data source '" + dsrc + "'");
                }
            ...
            if (this.debug) {
                logger.info(this.getClass() + ": putting new broker with identifier '" + dsrc + "'");
            }
    ...
    public final DatabaseInfo getMetaData(final String dsrc, final String usr, final String pwd) {
        ...
        if (usr == null) {
            if (this.debug) {
                logger.info(this.getClass() + ": user name is null, returning a similar broker for datasource '" + dsrc
                        + "'");
            }
            ...
            if (this.debug && (broker != null)) {
                logger.info(this.getClass() + ": returning a broker for datasource '" + dsrc + "', user name '" + usr
                        + "', password '" + pwd + "'");
            }
        }
    ...
    public final DbConnectionBroker get(final String dsrc, final String usr, final String pwd) {
        SQLMapBroker broker;
        // logger.info("In ConnectionBrokerManager.get(" + dsrc + "," + usr +
        // "," + pwd + ")");
        if (usr == null) {
            if (this.debug) {
                logger.info(this.getClass() + ": user name is null, returning a similar broker for datasource '" + dsrc
    ...
    public final void setDebug(final boolean b) {
        this.debug = b;
        if (this.debug) {
            logger.info(this.getClass() + "; debugging on");
        }
            ...
            if (broker != null) {
                try {
                    this.put(broker.datasource, broker.driver, broker.url, broker.username, broker.password,
                            broker.minconnections, broker.maxconnections, broker.logFile, broker.refresh,
                            broker.autocommit, true, broker.type);
     ...
     * 
     * @param datasource
     *            , name of the datasource that is searched.
     * @param donotremove
     *            , if set to true do not remove a found broker, even if it is
     *            dead or refresh is 0; use this flag if broker is simply used
     *            for template for creating new broker.
     * @return
     */
    ...
    /**
     * This function checks whether the resource is available (true) or
    ...
    public boolean isAvailable(String datasource) {
        ...
        SQLMapBroker broker = (this.brokerMap.get(datasource.replaceAll("'", "")));
        if (broker == null) {
            return true; // Try it to prevent deadlocking on changed web service
                         // that can never be reached due to former
                         // unavailability.
        }
    ...
    private class SQLMapBroker extends Object implements Cloneable {
        ...
        public SQLMapBroker(final String dsrc, final String drv, final String url, final String usr, final String pwd,
                final int minconn, final int maxconn, final String lfile, final double rfrsh, final Boolean ac,
                final String type) {
        ...
        @Override
        public Object clone() {
            final SQLMapBroker y = new SQLMapBroker(this.datasource, this.driver, this.url, this.username,
                    this.password, this.minconnections, this.maxconnections, this.logFile, this.refresh,
                    this.autocommit, this.type);
            return (y);
        }
