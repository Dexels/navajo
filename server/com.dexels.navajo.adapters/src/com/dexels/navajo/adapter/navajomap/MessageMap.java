...
public class MessageMap implements Mappable {
    ...
    public Date getDateProperty(String fullName) throws UserException {
        ...
        if (p != null) {
            if (p.getType().equals(Property.DATE_PROPERTY)) {
                ...
                throw new UserException(-1, "Invalid date property: " + fullName + "(string value = " + p.getValue()
                        + ", type = " + p.getType() + " )");
        } else
            propertDoesNotExistException(fullName);
        return null;
    ...
    public MessageMap getMessage() throws UserException {
        ...
        if (m == null)
            throw new UserException(-1,
                    "Could not find message: " + messagePointer + " in response document (" + msg.getName() + ")");
        else {
    ...
    public MessageMap[] getMessages() throws UserException {
        try {
            ...
            if ((all == null))
                throw new UserException(-1,
                        "Could not find messages: " + messagePointer + " in response document (" + msg.getName() + ")");
