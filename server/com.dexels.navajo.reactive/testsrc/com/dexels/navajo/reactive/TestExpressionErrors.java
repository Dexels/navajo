...
public class TestExpressionErrors {
	...
	@Test
	public void testMismatch() throws UnsupportedEncodingException, IOException {
		try( InputStream in = TestScript.class.getClassLoader().getResourceAsStream("msgtypemismatch.xml")) {
			StreamScriptContext myContext = TestSetup.createContext("Single",Optional.empty());
			try {
				ReactiveScript parsedScript = TestSetup.setup().parse(myContext.service, in,"serviceName");
				System.err.println("Problems: "+parsedScript.problems());
				...
				e.printStackTrace();
				int probCount = e.problems().size();
				Assert.assertEquals(1, probCount);
			}
		}
	}
	
	@Test
	public void testMissingSource() throws UnsupportedEncodingException, IOException {
		try( InputStream in = TestScript.class.getClassLoader().getResourceAsStream("missingsource.xml")) {
			StreamScriptContext myContext = TestSetup.createContext("Single",Optional.empty());
			try {
				ReactiveScript parsedScript = TestSetup.setup().parse(myContext.service, in,"serviceName");
				System.err.println("Problems: "+parsedScript.problems());
	...
	@Test
	public void testMissingTransformer() throws UnsupportedEncodingException, IOException {
		try( InputStream in = TestScript.class.getClassLoader().getResourceAsStream("missingtransformer.xml")) {
			StreamScriptContext myContext = TestSetup.createContext("Single",Optional.empty());
			try {
				ReactiveScript parsedScript = TestSetup.setup().parse(myContext.service, in,"serviceName");
				System.err.println("Problems: "+parsedScript.problems());
