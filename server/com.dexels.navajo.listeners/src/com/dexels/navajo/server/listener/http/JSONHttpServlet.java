...
public class JSONHttpServlet extends TmlHttpServlet {
	...
	private final void callDirect(HttpServletRequest request,
		...
		try {
			...
			Header header = NavajoFactory.getInstance().createHeader(tbMessage,
					service, username, password, expirationInterval);
	...
	private void writeJSON(Navajo n, OutputStreamWriter out, String callback,
		...
		try {
			...
			if (streamingMode) {
				...
				try {
					...
					fos.close();
					PrintWriter fw = new PrintWriter(new FileWriter(new File(f,
							"exception-" + logfileIndex)));
			...
			if (in != null
					...
					&& !Dispatcher.isSpecialwebservice(in.getHeader()
							.getRPCName())) {
				System.err.println(outDoc.getHeader().getHeaderAttribute(
						"accessId")
						+ ":"
