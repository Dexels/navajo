...
public class Cookie {
	...
	/**
	 * The constructor.Takes a string representing a cookie, parses, the string
	 * and populates itself with the corresponding data.
	 * 
	 * @param rawCookie
	 *            The string representing the cookie.
	 */
	public Cookie(String rawCookie) {
		...
		while (st.hasMoreElements()) {
			...
			if (tmpToken.toUpperCase().startsWith("EXPIRES")) {
				this.expires = tmpToken.substring(tmpToken.indexOf("=") + 1,
						tmpToken.length());
			} else if (tmpToken.toUpperCase().startsWith("DOMAIN")) {
				this.domain = tmpToken.substring(tmpToken.indexOf("=") + 1,
						tmpToken.length());
			} else if (tmpToken.toUpperCase().startsWith("PATH")) {
				this.path = tmpToken.substring(tmpToken.indexOf("=") + 1,
						tmpToken.length());
			} else {
	...
	/**
	 * Trims the initial space off of a string, if the string begins with a
	 * space.
	 * 
	 * @param value
	 ...
	 * 
	 * @return A string representing the key and value (e.g. foo=bar).
	 */
	...
	public String getValue() {
		return this.keyValue.substring(keyValue.indexOf("=") + 1,
				keyValue.length());
	}
	 ...
	 * 
	 * @return The expiration date. An empty string if none exists.
	 */
	 ...
	 * 
	 * @return The domain. An empty string if none exists.
	 */
	 ...
	 * 
	 * @return The path. An empty string if none exists.
	 */
