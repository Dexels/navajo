...
/**
 * This is a static utility class used to load a list of images or links or
 * whatever from the server. Used in Kafenio Applet for Image-from-Server and
 * HyperLink insertion
 * <p>
 ...
 * <li>The following lines should have a following form:
 * [name/description;;]partial_url
 * </ul>
 * <p>
...
public final class ListLoader {
	 ...
	 * 
	 * @param config
	 *            Kafenio configuration
	 * @param params
	 *            Parameters to pass to the Servlet/CGI script
	 * @param names
	 *            The returned list of names
	 * @param urls
	 *            The returned list of Urls
	 */
	private static void loadLists(KafenioPanelConfiguration config,
			String params, List names, List urls) {
		names.clear();
		urls.clear();
		...
		try {
			...
			for (int i = 1; i < strings.length; i++) {
				if (strings[i].indexOf(NAME_URL_SEPARATOR) >= 0) {
					names.add(strings[i].substring(0,
							strings[i].indexOf(NAME_URL_SEPARATOR)));
					urls.add(prefix
							+ Utils.ensureHasNoLeading('/', strings[i]
									.substring(strings[i]
											.indexOf(NAME_URL_SEPARATOR)
											+ NAME_URL_SEPARATOR.length())));
				} else {
					names.add(strings[i]);
			...
			log.error("NullPointerException in ListLoader: "
					+ npe.fillInStackTrace());
		} catch (IndexOutOfBoundsException ioob) {
			log.error("IndexOutOfBoundsException in ListLoader: "
					+ ioob.fillInStackTrace());
		} catch (MalformedURLException mue) {
			log.error("MalformedURLException in ListLoader: "
					+ mue.fillInStackTrace());
		} catch (UnsupportedEncodingException uee) {
			log.error("UnsupportedEncodingException in ListLoader: "
					+ uee.fillInStackTrace());
		} catch (IOException ioe) {
			log.error("IOException in ListLoader: " + ioe.fillInStackTrace());
		} catch (ClassNotFoundException cnfe) {
			log.error("ClassNotFoundException in ListLoader: "
	 ...
	 *            Kafenio configuration
	 * @param names
	 *            The returned list of image names
	 * @param urls
	 *            The returned list of image Urls
	 */
	public static void loadImages(KafenioPanelConfiguration config, List names,
			List urls) {
	 ...
	 *            The returned list of image Urls
	 */
	public static void loadLinks(KafenioPanelConfiguration config, List names,
			List urls) {
