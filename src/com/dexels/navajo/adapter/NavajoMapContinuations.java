...
public class NavajoMapContinuations extends NavajoMapUpdated {
	...
	public void setDoSend(final String method) throws UserException, ConditionErrorException, SystemException, AuthorizationException {
	  ...
	  protected  void scheduleAndContinue(String method, TmlRunnable onSuccess){
			prepareSend(method);
			TmlRunnable on2 = RunnableTools.prependRunnable(onSuccess, new Runnable(){
				@Override
				public void run() {
					continueAfterRun();
				}});
	...
	protected void scheduleExternalNavajoCall(String method, TmlRunnable on2) {
		try {
			System.err.println("Calling navajoMapContinuations. Using currentOutdoc: "+useCurrentOutDoc+"\nresolved out doc;");
			outDoc.write(System.err);
			  ...
			  System.err.println("STARTING MASSIVE DEBUG=========================");
			  ...
			  AsyncClient.getInstance().callService(this.access, outDoc, method, on2, null, new NavajoResponseCallback() {
				...
				public void responseReceived(Navajo n) {
					setResponseNavajo(n);
					  ...
					  try {
						NavajoMapContinuations.this.inDoc.write(System.err);
