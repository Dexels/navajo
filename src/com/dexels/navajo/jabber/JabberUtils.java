...
//			else
			...
			if ("muc#roomconfig_maxusers".equals(field.getVariable())) {
				...
				Iterator<String> it = form.getField("muc#roomconfig_maxusers").getValues();
				while (it.hasNext()) {
	...
	public static MultiUserChat joinRoom(XMPPConnection connection,String conferenceName, String roomName, String nickName, final Set<String> occupants)
			throws XMPPException {
	...
	private static void registerRoomListeners(String nickName, final MultiUserChat muc, final Set<String> occupants)  {
	...
	public static void main(String [] args) throws Exception {
		...
		ConnectionConfiguration config = new ConnectionConfiguration("spiritus", 5222);
		...
		Collection<HostedRoom> c = MultiUserChat.getHostedRooms(connection, "conference.dexels.nl");
		...
		for (HostedRoom hostedRoom : c) {
			...
			myMultiuserChat.addParticipantListener(new PacketListener(){
				...
				public void processPacket(Packet arg0) {
					System.err.println("IN addParticipantListener: " + arg0.getClass().getName());
					Presence pres = (Presence) arg0;
					System.err.println("WHO: " + pres.getFrom().substring(pres.getFrom().indexOf("/")+1));
					System.err.println(pres.toXML());
				}});
			//Thread.sleep(1000);
			
			
			System.err.println("occupants: " + myMultiuserChat.getOccupantsCount());
			...
			while ( occupants.hasNext() ) {
				...
				Chat chat = myMultiuserChat.createPrivateChat(okkie, new MessageListener() {
				...
				System.err.println("Sending message..." + chat.getThreadID());
				chat.sendMessage("postman");
				}
			}
		}
		//connection.login(username, password, NavajoClientFactory.getClient().getSessionToken());
		//joinRoom(connection, "conference.dexels.nl", "MyNavajoGroup5",  "MyNavajoServer3", new HashSet<String>());
	
		
//		MultiUserChat myMultiuserChat = new MultiUserChat(connection, "MyNavajoGroup@conference.dexels.nl");
