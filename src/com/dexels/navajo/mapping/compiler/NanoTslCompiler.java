...
//      System.err.println("getNextElement node is experimental and should not be trusted!");
...
//    return null;
  ...
  public String expressionNode(int ident, XMLElement exprElmnt, int leftOver,
...
//        value = child.getContent();
        ...
        throw new TslCompileException(TslCompileException.TSL_MISSING_VALUE,"Error @" +
                            (exprElmnt.getParent() + "/" + exprElmnt) + ": <expression> node should either contain a value attribute or a text child node: >" +
                            value + "<",exprElmnt.getStartOffset(),exprElmnt.getOffset());
      }
    }
...
//    System.err.println(">> "+removeNewLines(value)+" <<");
        ...
        if (!isStringOperand) {
          ...
          // TODO:::::::::::::: SHOULDN'T THERE BE AN OPTIMIZE EXPRESSION HERE AS WELL?
  ...
  public String methodsNode(int ident, XMLElement n) {
      ...
      if (((XMLElement)children.get(i)).getName().equals("method")) {
        ...
        for (int j = 0; j < required.size(); j++) {
          if (((XMLElement)required.get(j)).getName().equals("required")) {
            String reqMsg = ( (XMLElement) required.get(j)).getNonNullStringAttribute(
                "message");
  ...
  public String messageNode(int ident, XMLElement n, String className, String objectName) throws Exception {
    ...
    if (nextElt != null && nextElt.getName().equals("map") &&
        nextElt.getNonNullStringAttribute("ref") != null &&
...
result.append(printIdent(ident + 4) +
      ...
      } catch (Exception e) { throw new TslCompileException(TslCompileException.TSL_UNKNOWN_MAP,"Could not find adapter: " + subClassName,nextElt); }
      ...
      Vector children = n.getChildren();
      for (int i = 0; i < children.size(); i++) {
        result.append(compile(ident + 2, ((XMLElement)children.get(i)), className,
                              objectName));
      }
    }
  ...
  public String propertyNode(int ident, XMLElement n, boolean canBeSubMapped, String className, String objectName) throws
      Exception {
    StringBuffer result = new StringBuffer();
    ...
    for (int i = 0; i < children.size(); i++) {
      ...
      if (((XMLElement)children.get(i)).getName().equals("expression")) {
        result.append(expressionNode(ident, (XMLElement) children.get(i),
      ...
      else if (((XMLElement)children.get(i)).getName().equals("map")) { // ABout to map a "selection" property!!!
        if (!canBeSubMapped) {
          throw new TslCompileException(TslCompileException.SUB_MAP_ERROR,"This property can not be submapped: " +
                              propertyName,n);
        }
        if (!type.equals("selection")) {
          throw new TslCompileException(TslCompileException.SUB_MAP_ERROR,"Only selection properties can be submapped: " +
                              propertyName,n);
        }
        ...
        throw new TslCompileException(TslCompileException.SUB_MAP_ERROR,"Illegal child tag <" + ((XMLElement)children.get(i)).getName() + "> in " + tagValue + " (Check your script) ",n);
      }
    }
    ...
    if (isMapped) {
      ...
      try {
      contextClass = Class.forName(className, false, loader);
      } catch (Exception e) { throw new TslCompileException(TslCompileException.TSL_UNKNOWN_MAP,"Could not find adapter: " + className,n); }
      ...
      for (int i = 0; i < children.size(); i++) {
        if (((XMLElement)children.get(i)).getName().equals("property")) {
          ...
          if (! (subPropertyName.equals("name") ||
            ...
            throw new TslCompileException(TslCompileException.SUB_MAP_ERROR, "Only 'name' or 'value' named properties expected when submapping a 'selection' property",elt);
          }
          ...
          for (int j = 0; j < expressions.size(); j++) {
            ...
            if ( (expressions.get(j)instanceof XMLElement) &&
                ((XMLElement)expressions.get(j)).getName().equals("expression")) {
              result.append(expressionNode(ident + 4,
                                           (XMLElement) expressions.get(j),
          ...
          throw new TslCompileException(TslCompileException.SUB_MAP_ERROR,
              "<property> tag expected while sub-mapping a selection property: " + ((XMLElement)children.get(i)).getName(),(XMLElement)children.get(i));
        }
      }
  ...
  public String fieldNode(int ident, XMLElement n, String className,
    ...
    for (int i = 0; i < children.size(); i++) {
      // Has condition;
      if (((XMLElement)children.get(i)).getName().equals("expression")) {
        result.append(expressionNode(ident + 2, (XMLElement) children.get(i),
                                     --exprCount, className, objectName));
      }
...
//      try {
		...
		try {
		 contextClass = Class.forName(className, false, loader);
			    ...
			    throw new TslCompileException(TslCompileException.TSL_UNKNOWN_MAP,"Could not find adapter: " + className,n.getParent().getStartOffset(),n.getOffset()); }
        String type = null;
        try {
        	type = MappingUtils.getFieldType(contextClass, attribute);
        } catch (Exception e) { throw new TslCompileException(TslCompileException.TSL_UNKNOWN_FIELD,"Could not find field: " + attribute + " in adapter " + className,n.getStartOffset(),n.getOffset()); }
...
//      }
  ...
  private final void includeNode(String scriptPath, XMLElement n, XMLElement parent) throws Exception {
    ...
    if (script == null || script.equals("")) {
      throw new TslCompileException(TslCompileException.VALIDATION_NO_SCRIPT_INCLUDE, "No script name found in include tag (missing or empty script attribute)",n);
    }
...
//    System.err.println("Compiling includes. Dont really trust the mechanism.");
...
//    System.err.println("AFTER INCLUDE: "+parentNode.toString());
  ...
  public String compile(int ident, XMLElement n, String className, String objectName) throws
      Exception {
    StringBuffer result = new StringBuffer();
...
//    }
    ...
    else if (n.getName().equals("message") || 
    		(n.getName().equals("param") && ((XMLElement) n).getNonNullStringAttribute("type").equals("array")  )) {
 ...
 private final void generateValidations(XMLElement d, StringBuffer generatedCode ) throws Exception {
   ...
   for (int i = 0; i < list.size(); i++) {
    ...
    if (rule.isCalled("check")) {
         ...
         if (rule.equals("")) {
           throw new TslCompileException(TslCompileException.VALIDATION_NO_CODE, "Validation syntax error: code attribute missing or empty",rule);
         }
         if (value.equals("")) {
           throw new TslCompileException(TslCompileException.VALIDATION_NO_VALUE,"Validation syntax error: value attribute missing or empty",rule);
         }
...
//         System.err.println(" Appended nr: "+j+" size: "+rules.size());
...
//       if (j != ( rules.size() - 2 ) ) { // Add ","
  ...
  public void compileScript(String script, String scriptPath, String workingPath, String packagePath) throws
      SystemException, TslCompileException {
...
//       System.err.println("Parsing: "+script+" took: "+(System.currentTimeMillis()-cc)+" millis.");
//      cc = System.currentTimeMillis();
//      tslDoc.parseFromReader(new InputStreamReader(new FileInputStream(scriptPath + "/" + packagePath + "/" + script + ".xml")));
//           XMLDocumentUtils.createDocument(new FileInputStream(scriptPath + "/" + packagePath + "/" + script + ".xml"), false);
...
//      tslDoc.disposeAll();
//      System.err.println("Compiling: "+script+" took: "+(System.currentTimeMillis()-cc)+" millis.");
    } catch (Exception e) {
      e.printStackTrace();
...
//  public void finalize() {
    //System.out.println("FINALIZE() METHOD CALL FOR NanoTslCompiler OBJECT " + this);
//  }
...
public static void main(String[] args) throws Exception {
   ...
   if (args.length == 0) {
     System.out.println("NanoTslCompiler: Usage: java com.dexels.navajo.mapping.compiler.NanoTslCompiler <scriptDir> <compiledDir> [-all | scriptName]");
