    ...
    private void stripMessage(Node n) {
        ...
        for (int i = 0; i < propertyList.getLength(); i++) {
            ...
            if (node.getNodeName().equals(Property.PROPERTY_DEFINITION)) {
                m = (Element) node;
                try {
                    if ((at = m.getAttributeNode(Property.PROPERTY_DESCRIPTION))
                            != null)
                        m.removeAttributeNode(at);
                    if ((at = m.getAttributeNode(Property.PROPERTY_LENGTH))
                            != null)
                        m.removeAttributeNode(at);
                    if ((at = m.getAttributeNode(Property.PROPERTY_CARDINALITY))
                            != null)
                        m.removeAttributeNode(at);
                } catch (Exception ex1) {}
                try {
