...
public class TipiRunScript extends TipiAction {
	public void execute(TipiEvent event) throws com.dexels.navajo.tipi.TipiException, com.dexels.navajo.tipi.TipiBreakException {
		...
		String engine = (String) getEvaluatedParameterValue("engine", event);
	 	...
	 	scr.put("component", ((TipiComponentImpl)getComponent()).createAdapter(this, event));
	 	TipiComponentImpl homeComponent = (TipiComponentImpl)getComponent().getHomeComponent();
	 	if(homeComponent!=null) {
			scr.put("home", homeComponent.createAdapter(this, event));
	 	}
	 	TipiComponentImpl defaultTopLevel = (TipiComponentImpl)getComponent().getContext().getDefaultTopLevel();
	 	TipiComponentImpl frame = (TipiComponentImpl) defaultTopLevel.getTipiComponent("init");
	 	BaseAdapter root = frame.createAdapter(this, event);
		scr.put("root", root);
	 	scr.put("params", event.getEvaluatedParameters());
		
	 	System.err.println("Server spinup: "+(System.currentTimeMillis() - startup));

//		String adapterPath = getContext().getT DispatcherFactory.getInstance().getNavajoConfig().getAdapterPath();
...
//	 			System.err.println("Compile took: "+(System.currentTimeMillis() - start));
//	 		 	start = System.currentTimeMillis();
//	 		 	ccc.eval();		
//	 			System.err.println("Run took: "+(System.currentTimeMillis() - start));
//	 		}
//			System.err.println("Compilabel: "+compilable);
	 		...
	 		System.err.println("Eval took: "+(System.currentTimeMillis() - start));
	 	} catch (ScriptException e) {
			System.err.println("Scripting engine fail: "+e.getMessage());
			throw new TipiException("Scripting engine fail: "+e.getMessage(), e);
		}
	}
	...
	private void appendActions(String actionClass,TipiEvent event, ScriptEngine scr, TipiExtension t, boolean functions) {
		...
		try {
			...
			scr.put(t.getProjectName().toLowerCase()+(functions?"function":""), b);
		 	
		} catch (ClassNotFoundException e) {
			System.err.println("No actions for extension: "+t.getProjectName());
