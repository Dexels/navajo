...
public abstract class BaseAdapter {
	...
	public List<BaseAdapter> getChildren() {
		...
		for (TipiComponent tc : myComponent.getChildren()) {
			result.add(((TipiComponentImpl) tc).createAdapter(invocation,event));
		}
		return result;
	}
	
	public BaseAdapter getChild(String id) {
		 TipiComponentImpl rr =  (TipiComponentImpl) myComponent.getTipiComponent(id);
		if(rr==null) {
			return null;
		}
	...
	public BaseAdapter getComponent(String path) {
		 TipiComponentImpl rr =  (TipiComponentImpl) myComponent.getTipiComponentByPath(path);
		if(rr==null) {
			return null;
		}
		return rr.createAdapter(invocation, event);
