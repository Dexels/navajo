...
/**
 * <p>Title: VerticalFlowLayout</p>
 * <p>Description: Vertical implementation of the wellknown FlowLayout</p>
...
public class VerticalFlowLayout
  ...
  public VerticalFlowLayout(String location)  {
    ...
    if (location.equals(BorderLayout.EAST) || location.equals(BorderLayout.WEST)) {
      // We're fine
    }
    else {
      throw new IllegalArgumentException("Location must be one of: BorderLayout.EAST or BorderLayout.WEST");
    }
  }
  ...
  public void layoutContainer(Container obj) {
    ...
    while (it.hasNext()) {
      ...
      if (columns == 1) {
        ...
        current.setBounds(0, y_pos, current.getPreferredSize().width + 2, current.getPreferredSize().height);
        current.setVisible(true);
      }
      else {
        if (location.equals(BorderLayout.EAST)) {
          int multiplier = column - 1;
          current.setBounds( (current.getPreferredSize().width * multiplier), y_pos, current.getPreferredSize().width + 2, current.getPreferredSize().height);
          current.setVisible(true);
        }
        else {
          int x_pos = ( (columns - 1) * current.getPreferredSize().width) - ( (column - 1) * current.getPreferredSize().width);
          //System.err.println("adding: " + x_pos + ", " + y_pos + " , cols: " + columns + ", col: " + column);
          current.setBounds(x_pos, y_pos, current.getPreferredSize().width + 2, current.getPreferredSize().height);
          current.setVisible(true);
        }
      }
    }
