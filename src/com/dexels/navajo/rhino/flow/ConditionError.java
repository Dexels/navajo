...
public class ConditionError extends RuntimeException {
	...
	public ConditionError() {
		...
		conditionMessage = NavajoFactory.getInstance().createMessage(conditionNavajo, "ConditionErrors", Message.MSG_TYPE_ARRAY);
		try {
	...
	public void append(int code, String description) {
		Message m = NavajoFactory.getInstance().createMessage(conditionNavajo, "ConditionErrors", Message.MSG_TYPE_ARRAY_ELEMENT); 
		conditionMessage.addElement(m);
		try {
			Property codeProperty = NavajoFactory.getInstance().createProperty(conditionNavajo, "Code", Property.INTEGER_PROPERTY, ""+code, 0, "", Property.DIR_OUT);
			Property descriptionProperty = NavajoFactory.getInstance().createProperty(conditionNavajo, "Description", Property.STRING_PROPERTY, description, 0, "", Property.DIR_OUT);
