...
public class RhinoRunner {
	...
	public static void main(String[] args) throws IOException, InterruptedException, NavajoException, UserException, SystemException, AuthorizationException {
		...
		t.run(a);
		a.getOutputDoc().write(System.err);
	//	Thread.sleep(60000);
	}
...
//	    	  continueScript(pending);
	  ...
	  public InputStream getScriptStream(String script) throws IOException {
			if(localMode ) {
				...
				File compiledPath = new File(DispatcherFactory.getInstance().getNavajoConfig().getCompiledScriptPath()+"/"+script+".js");
				FileInputStream fis = new  FileInputStream(compiledPath);
				return fis;
			}
		}
			
	  public void run(Access access) throws NavajoException, UserException {
			final ByteArrayOutputStream baos = new ByteArrayOutputStream();
			InputStream is = null;
			try {
				is = getScriptStream(access.getRpcName());
				 runBlockingScript(is, access, baos, new Runnable() {
					public void run() {
