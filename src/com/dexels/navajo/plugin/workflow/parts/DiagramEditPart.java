...
private static class ShapesXYLayoutEditPolicy extends XYLayoutEditPolicy {
	...
	protected Command createChangeConstraintCommand(ChangeBoundsRequest request,
		...
		if (child instanceof StateEditPart && constraint instanceof Rectangle) {
			...
			return new ShapeSetConstraintCommand(
					(StateElement) child.getModel(), request, (Rectangle) constraint);
		}
	...
	protected Command getCreateCommand(CreateRequest request) {
		...
		if (childClass == StateElement.class) {
			...
			return new ShapeCreateCommand((StateElement)request.getNewObject(), 
					(WorkflowElement)getHost().getModel(), (Rectangle)getConstraintFor(request));
		}
		return null;
	}
