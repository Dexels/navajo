 ...
 * configuration of path information</p>
...
public class PathConfig {
  ...
  public static final Logger logger =
      Logger.getLogger(PathConfig.class.getName());
  ...
  /**
   * get the path property given the property's name
   * path property always include the base directory
   * @param String name of path property
  ...
  private void setBase() throws ConfigurationException {
    ...
    final String x = System.getProperty(NavaDocConstants.BASE_SYS_PROPERTY);
    if ( (x != null) && (x.length() > 0)) {
      this.logger.log(Priority.DEBUG, "base directory system property set to '" +
                      x + "'");
      ...
      final String y = this.elem.getAttribute(NavaDocConstants.BASE_ATTR);
  ...
  private void setPaths() throws ConfigurationException {
      ...
      if (l.item(i).getNodeType() == Node.ELEMENT_NODE) {
        ...
        if ( (value != null) && (value.length() > 0)) {
          ...
          if (name.equals(NavaDocConstants.SVC_PATH_ELEMENT) &&
            ...
            throw new ConfigurationException("path directory for property '" +
                                             name +
                                             "' set to '" + value +
                "' but it is not a valid directory",
                this.configUri);
          }
          else if (name.equals(NavaDocConstants.STYLE_PATH_ELEMENT) &&
          ...
          else if (name.equals(NavaDocConstants.TARGET_PATH_ELEMENT) &&
                   !path.exists()) {
            if (!path.mkdir()) {
              throw new ConfigurationException("path directory for property '" +
                                               name +
                  "' does not exist and could not create it", this.configUri);
            } else {
              this.logger.log( Priority.DEBUG, "created directory '" + path + "'" );
            }
          }
          else if (name.equals(NavaDocConstants.TARGET_PATH_ELEMENT) &&
                   path.exists() && !path.isDirectory()) {
          ...
          this.logger.log(Priority.DEBUG,
                          "'" + name + "' directory set to '" + path.toString() +
                          "'");
    ...
    final String[] required = {
        NavaDocConstants.SVC_PATH_ELEMENT, NavaDocConstants.STYLE_PATH_ELEMENT,
