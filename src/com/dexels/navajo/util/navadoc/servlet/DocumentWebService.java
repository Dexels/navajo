 ...
 * servet to document a given web service from a given document set on the fly,
 * Requires the following params: sname = web service name, set = document
 * set name</p>
...
public class DocumentWebService
  ...
  public static final Logger logger =
      Logger.getLogger(DocumentWebService.class.getName());
  ...
  public void init() throws ServletException {
    ...
    this.logger.log(Priority.DEBUG, "basic logging started");

    this.configUri = this.getInitParameter(NavaDocConstants.
                                           WEB_CONFIG_INITPARAM);
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response) throws
    ...
    if ( (set == null) || (set.length() == 0)) {
      set = (String) sess.getAttribute(NavaDocConstants.WEB_SET_PARAMETER);
    }
    if ( (set == null) || (set.length() == 0)) {
      dset = this.config.getDefaultSet();
      if (dset == null) {
        throw new ServletException("could not determine default document set");
      }
      set = dset.getName();
    }
    else {

      dset = (DocumentSet)this.config.getDocumentSetMap().get(
          set);
      if (dset == null) {
        throw new ServletException("provided invalid document set '" + set +
                                   "'");
      }
    }

    final NavaDocTransformer transformer = this.getTransformer(dset);
    final ServicesList list = (ServicesList)this.slistMap.get(dset.getName());
    if (list == null) {
    ...
    if ( (sname != null) && (sname.length() > 0)) {
      if (!list.contains(sname)) {
        throw new ServletException("'" + sname + "' web service not found in '" +
                                   set + "'");
      }

      transformer.transformWebService(sname);
      final NavaDocOutputter outputter = new NavaDocOutputter(transformer,
          response.getWriter());
      return;
    }
    else {
      ...
      final NavaDocOutputter outputter = new NavaDocOutputter(idx,
          response.getWriter());
    }
  ...
  private NavaDocTransformer getTransformer(final DocumentSet dset) throws
      ServletException {
  ...
  private synchronized void setCache(final String name,
                                     final NavaDocTransformer t,
                                     final ServicesList l) {
    this.transformMap.put(name, t);
    this.slistMap.put(name, l);
    this.logger.log(Priority.DEBUG,
                    "cached services list and transfer for document set '" +
                    name + "'");
  }
   ...
   * @throws ServletException if there's any trouble reading services notes
   */

  private NavaDocIndexDOM getIndexDOM(DocumentSet dset, ServicesList list,
                                      NavaDocTransformer transformer,
                                      HttpServletRequest request) throws
      ServletException {
    NavaDocIndexDOM idx = (NavaDocIndexDOM)this.indexMap.get(dset.getName());
    if (idx == null) {
      try {
