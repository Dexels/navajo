...
public class MultiUserNavajoClient {
	...
	private static final Map<String,Long> disabledServers = new HashMap<String,Long>();
	...
	public void remoteDispatch(String server, HttpServletRequest request,
		...
		try {
			java.lang.reflect.Method timeout = con.getClass().getMethod(
		...
		try {
			...
			System.err
					.println("setChunkedStreamingMode does not exist, upgrade to java 1.5+");
		}
	...
	protected InputStream doTransaction(String name, Navajo d,
			HttpURLConnection myCon) throws IOException, ClientException,
			NavajoException, javax.net.ssl.SSLHandshakeException {
		URL url;
	...
	public final Navajo doSimpleSend(Navajo out, String server)
		...
		try {
			...
			System.err.println("Disabled server: " + server + " for "
		...
		if (!pingStarted) {
			new Thread() {
				public void run() {
					...
					Header outHeader = NavajoFactory.getInstance()
							.createHeader(out, "navajo_logon", "ROOT", "", -1);
					...
					while (!killed) {
						try {
							...
							for (int i = 0; i < serverUrls.length; i++) {
								try {
									HttpURLConnection myCon = null;
									InputStream in = doTransaction(
											serverUrls[i], out, myCon);
									Navajo n = NavajoFactory.getInstance()
											.createNavajo(in);
									in.close();
									Header h = n.getHeader();
									String load = h
											.getHeaderAttribute("cpuload");
	...
	private final void switchServer() {
		...
		for (int i = 0; i < serverUrls.length; i++) {
			if (serverLoads[i] < minload && serverLoads[i] != -1.0) { // If
																		// there
																		// is
																		// really
																		// a
		...
		System.err.println("currentServer = " + serverUrls[currentServerIndex]
		...
		if (disabledServers.containsKey(nextServer)) {
			...
			if (t + serverDisableTimeout < System.currentTimeMillis()) {
				...
				System.err.println("Reinstating server: " + nextServer
