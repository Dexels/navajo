...
public class NavajoAgent
  ...
  public NavajoAgent(ResourceBundle navajoAgentProperties) {
    ...
    enableCache = (navajoAgentProperties.getString("tml.cache.enable").equals(
        "yes"));
    enableHttps = (navajoAgentProperties.getString("navajo.security.https").
                   equals("yes"));
  ...
  private boolean readFromCache(String method, Navajo message,
    ...
    Navajo doc = null;
    try {
      input = new FileInputStream(new File(cachePath + method + identifier +
                                           ".xml"));
      doc = NavajoFactory.getInstance().createNavajo(input);
      message.appendDocBuffer(doc.getMessageBuffer());
  ...
  private boolean writeToCache(String method, Navajo message, String identifier) throws
      NavajoException {
    ...
    try {
      ...
      for (int i = 0; i < messages.size(); i++) {
        ...
        Message msg = message.copyMessage( (Message) messages.get(i),
                                          outMessage);
  ...
  public void send(String method, Navajo message, boolean useCache,
                   boolean checkMethod, long expirationInterval) throws
   ...
   */
  public void send(String method, Navajo message, boolean useCache,
                   boolean checkMethod, long expirationInterval,
  ...
  public void send(String method, Navajo message, boolean useCache,
    ...
    try {
      ...
      if (!useCache
        ...
        doMethod(method, username, password, message, navajoServer, enableHttps,
                 keystore,
