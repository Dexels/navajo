	...
	public void init() {
		...
		try {
			...
			for (int i = 0; i < children.size(); i++) {
				...
				Vector<XMLElement> def = children.get(i).getChildren();
				String name = null;
				String object = null;
				String description = null;
				String usage = null;
				for (int j = 0; j < def.size(); j++) {
					if ( def.get(j).getName().equals("name")) {
						name =  def.get(j).getContent();
					}
					if ( def.get(j).getName().equals("object")) {
						object =  def.get(j).getContent();
					}
					if ( def.get(j).getName().equals("usage")) {
						usage =  def.get(j).getContent();
					}
				...
				if ( name != null ) {
					fuds.put(name, new FunctionDefinition(object, description, usage));
				}
			}
			setConfig(fuds);
