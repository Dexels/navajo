...
public class PersistenceManagerImpl implements PersistenceManager {
  ...
  private synchronized Configuration readConfiguration(String fileName)  {
    try {
      ...
      c.statistics = ((config.getProperty("/persistence-manager/statistics") != null) &&
                      (config.getProperty("/persistence-manager/statistics").getValue().equals("on")));
      if ((c.statistics) && (config.getProperty("/persistence-manager/statistics") != null)) {
  ...
  private synchronized void statistics(long start, long end, boolean inCache) {
     long total = (end - start);
      ...
      double cptVar =((totalCacheProcessing * totalCacheProcessing)/(double)cachehits - totalCacheProcessing2/(double)cachehits)/1000000.0;

      double ocpt = (this.totalOutOfCacheProcessing/(double)(totalhits - cachehits))/(1000.0); // Average out of cache processing time.
      double ocptVar =((totalOutOfCacheProcessing * totalOutOfCacheProcessing)/(double)(totalhits - cachehits) -
                                  totalOutOfCacheProcessing2/(double)(totalhits - cachehits))/1000000.0;
      ...
      if (cachehits > 0) {
        //System.out.println("Average in cache processing time: " + cpt + " secs. (" + cptVar + ")" );
      }
