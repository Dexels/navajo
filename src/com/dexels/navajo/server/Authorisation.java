...
*/
      ...
      // Insert new user.
      String query2 = "INSERT INTO users (id, name, password) VALUES (" + userId + ", '" + user + "', '" + password + "')";
...
//      stmt.close();
                     ...
                     throws SystemException
  {
    String query = "INSERT INTO access (id, user_id, service_id, entered, tijdstip, address, host, user_agent) values "+
                   "(?, ?, ?, " + this.getCurrentDateQuery() + ", " + this.getCurrentTimeQuery() + ", ?, ?, ?)";
    ...
    try {
      ...
      if (con == null) // Oops. lost connection or something, reinitialize broker.
      {
        ...
        try {
          ...
          //System.err.println("failed: " + cnfe.getMessage());
        }
  ...
  public Vector showLogView(Access access, Vector users, Vector services,
                                   Vector levels,
                                   boolean allUsers, boolean allServices,
    ...
    String allQuery = "SELECT a.entered, u.name, s.name, g.name, l.comment, a.tijdstip, a.user_agent, a.address, a.host FROM " +
                        "access a, log l, users u, services s, log_level g WHERE " +
                        "l.access_id=a.id AND u.id = l.user_id AND s.id = l.service_id AND " +
                        "l.log_level=g.id AND " +
                        "a.entered >= \"" + beginDate +"\" AND a.entered <= \"" + endDate + "\"";
    Connection con = null;
    ...
    try {
      ...
      if (!allUsers) {
        userWhere = " AND (";
        if (users.size() < 1)
          userWhere += "l.user_id = -1";
        for (int i = 0; i < users.size(); i++) {
          int id = getUserId(con, (String) users.get(i));
          userWhere += "l.user_id = " + id;
          if (i < users.size() - 1)
            userWhere += " OR ";
        }
        userWhere += ")";
      }
      //Util.debugLog(2, "userWhere = " + userWhere);

      if (!allServices) {
        serviceWhere = " AND (";
        if (services.size() < 1)
          serviceWhere += "l.service_id = -1";
        for (int i = 0; i < services.size(); i++) {
          int id = getServiceId(con, (String) services.get(i));
          serviceWhere += "l.service_id = " + id;
          if (i < services.size() - 1)
            serviceWhere += " OR ";
        }
        serviceWhere += ")";
      }
      //Util.debugLog(2, "serviceWhere = " + serviceWhere);


      levelWhere = "AND (";
      if (levels.size() < 1)
        levelWhere += "l.log_level = -1";
      for (int i = 0; i < levels.size(); i++) {
          levelWhere += "l.log_level = " + levels.get(i);
          if (i < levels.size() - 1)
            levelWhere += " OR ";
      }
      levelWhere += ")";
      //Util.debugLog(2, "levelWhere = " + levelWhere);

      query += allQuery + userWhere + serviceWhere + levelWhere;
      //Util.debugLog(2, "Query = " + query);
      ...
      while (rs.next()) {
        ...
        ld.entered = Util.getDate(datum);
        ld.user = rs.getString(2);
        ld.rpc = rs.getString(3);
        ld.level = rs.getString(4);
        ld.comment = rs.getString(5);
        ld.timestamp = rs.getString(6);
        ld.userAgent = rs.getString(7);
        ld.ipAddress = rs.getString(8);
        ld.hostName = rs.getString(9);
        result.add(ld);
      }
...
/*
  public Vector showParameters(Access access, String userName) throws SystemException {
...
//      Access access = new Access(78, 1, 2, "ARJEN", "AAP", "", "192.65.323.1", "piupohost", myBroker, Authorisation.DBMS_MSSQL);
    ...
    for(int i=0; i<a.size(); i++){
      //System.err.println("service: "+(String)a.get(i));
    }
  ...
  public String simpleSelect(Access access, String target, String table, String keyName, String key) throws SystemException {
    ...
    String query = "SELECT "+ target +" FROM " + table +" where "+ keyName +" = '"+key+"'";
  ...
  public Vector[] select(Access access, String[] targets, String table, String[] whereColumNames, String[] whereColumValues) throws SystemException {
    ...
    if(whereColumNames!=null){
      ...
      for(int i=0; i<aantalWhereNames; i++){
        where=where+whereColumNames[i] + " = " + "'" + whereColumValues[i] +"'";
        if(i<(aantalWhere-1)){
          where=where+" and ";
        }
      }
...
*/
    if(aantalSetNames != aantalSetValues){
      //System.err.println("\n\n\n !!!WARRING!!! for SET: aantal columNames != aantal columValues \n\n\n");
    }
    if(aantalWhereNames != aantalWhereValues){
