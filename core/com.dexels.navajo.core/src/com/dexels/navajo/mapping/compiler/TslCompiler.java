...
public class TslCompiler {
	    ...
	    for (int i = 0; i < children.getLength(); i++) {
	      if (children.item(i).getNodeName().equals("operation")) {
	        ...
	        String operationString = 
	        		"com.dexels.navajo.document.Operation o = " + 
	                "NavajoFactory.getInstance().createOperation(access.getOutputDoc(), " + 
	        				"\"" + method  + "\", \"" + service + "\", \"" + entity + "\", null);\n";
	        ...
	        if ( extraMessageName != null ) {
	        	...
	        	String extraNavajoOperation = 
	        			"Navajo extra = NavajoFactory.getInstance().createNavajo(new java.io.StringReader(\"" + extraNavajo + "\"));\n" +
	...
	public String messageNode(int ident, Element n, String className,
		...
		if (nextElt != null && nextElt.getNodeName().equals("map")
			...
			forceArray = type.equals(Message.MSG_TYPE_ARRAY); // (
																// nextElt.getAttribute("forcearray")
																// != null &&
																// !nextElt.getAttribute("forcearray").equals("")
																// );
	...
	public String fieldNode(int ident, Element n, String className,
		...
		if (!isMapped) {
			...
			// System.err.println("TYPE FOR " + attribute + " IS: " + type +
			// ", ARRAY = " + isArray);
			...
			// if (!isArray && !MappingUtils.isMappable(contextClass,
			// attribute,loader)) {
			// throw new TslCompileException(-1, "Not a mappable field: " +
			// attribute, 0, 0);
			// }
	...
	public String compile(int ident, Node n, String className,
		 ...
		 * System.err.println("in compile(), node name = " + n.getNodeName() +
		 * ", attribute name = " + ((Element) n).getAttribute("name")); } else {
		 * System.err.println("in compile(), node not of type element " +
		 * n.getNodeName()); }
		 */
	...
	private void compileStandAlone(boolean all, String script, String input,
			String output, String packagePath, String[] extraclasspath,
			String configPath, List<Dependency> deps, String tenant,
		...
		try {
			...
			try {
				...
				Writer w = navajoIOConfig.getOutputWriter(output, packagePath,
						script, ".java");
