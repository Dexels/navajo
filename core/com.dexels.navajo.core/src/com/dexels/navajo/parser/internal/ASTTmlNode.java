...
public final class ASTTmlNode extends SimpleNode {
	...
	public final Object interpret() throws TMLExpressionException {
				...
				if (val.indexOf(Navajo.MESSAGE_SEPARATOR) != -1) {
					if (doc == null) {
						throw new NullPointerException(
								"Can't evaluate TML node: No parent message and no document found.");
					}
		...
		for (int j = 0; j < match.size(); j++) {
			...
			if (!exists && (prop == null))
				if (parentMsg != null) {
					throw new TMLExpressionException("TML property does not exist: " + val + " parent message: "
							+ parentMsg.getFullMessageName());
				} else {
			...
			if (value == null && !type.equals(Property.SELECTION_PROPERTY)) { // If value attribute does not exist AND
																				// property is not selection property
																				// assume null value
				resultList.add(null);
						...
						try {
							...
							if (option.equals("month")) {
								...
								throw new TMLExpressionException(
										"Option not supported: " + option + ", for type: " + type);
							}
							try {
