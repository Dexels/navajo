...
public class TslPreCompiler {
	...
	protected void getAllDependencies(String scriptFile, String scriptTenant, String scriptFolder,
			List<Dependency> deps, Document tslDoc) throws UserException, XPathExpressionException {
	...
	protected void findMethodDependencies(String scriptFile, String scriptTenant, String scriptFolder,
			List<Dependency> deps, Document tslDoc) {
		...
		for (int i = 0; i < methods.getLength(); i++) {
			...
			deps.add(
					new Dependency(scriptFile, methodScriptFile, Dependency.METHOD_DEPENDENCY, getLineNr(n), isBroken));
	...
	protected void findEntityDependencies(String scriptFile, String scriptTenant, String scriptFolder,
			List<Dependency> deps, Document tslDoc) {
		...
		for (int i = 0; i < operations.getLength(); i++) {
			...
			if (scriptTenant != null) {
				// trying tenant-specific variant first
				String operationScriptFile = scriptFolder + File.separator + operationScript + "_" + scriptTenant
						+ ".xml";
			...
			if (operationValidationScriptFile != null) {
				...
				deps.add(new Dependency(scriptFile, operationValidationScriptFile, Dependency.ENTITY_DEPENDENCY,
						getLineNr(n), isBroken));
		...
		for (int i = 0; i < messages.getLength(); i++) {
			...
			for (String superEntity : superEntities) {
				...
				if (scriptTenant != null) {
					// trying tenant-specific variant first
					String operationScriptFile = scriptFolder + File.separator + superEntity + "_" + scriptTenant
							+ ".xml";
				...
				String superScriptFile = scriptFolder + File.separator + "entity" + File.separator + superEntity
						+ ".xml";
	...
	protected void findIncludeDependencies(String fullScriptPath, String scriptTenant, String scriptFolder,
			List<Dependency> deps, Document tslDoc) throws UserException {
	...
	protected void findMapDependencies(String scriptFile, String scriptTenant, String scriptFolder,
			List<Dependency> deps, Document tslDoc) throws XPathExpressionException {
		...
		NodeList nodes = (NodeList) xPath.evaluate(
				"//map[@object='com.dexels.navajo.adapter.NavajoMap']/field[@name='doSend']/expression",
		...
		for (int i = 0; i < nodes.getLength(); ++i) {
			...
			if (navajoScript.contains("@")) {
				...
				for (String res : result) {
					addScriptDependency(scriptFile, scriptTenant, deps, res, scriptFolder, getLineNr(expression),
							Dependency.NAVAJO_DEPENDENCY);
				}

			} else if (navajoScript.startsWith("[/")) {
				// The navajo script is retrieved from the Indoc or database
				// result - not supported
				deps.add(new Dependency(scriptFile, "scripts/__unknown__.xml", Dependency.UNKNOWN_TYPE,
						getLineNr(expression)));
			} else {
				addScriptDependency(scriptFile, scriptTenant, deps, navajoScript, scriptFolder, getLineNr(expression),
						Dependency.NAVAJO_DEPENDENCY);
			}
		}
		...
		nodes = (NodeList) xPath.evaluate(
				"//map[@object='com.dexels.navajo.entity.adapters.EntityMap']/field[@name='name']/expression",
				tslDoc.getDocumentElement(), XPathConstants.NODESET);
	...
	private void addScriptDependency(String scriptFile, String scriptTenant, List<Dependency> deps, String navajoScript,
			String scriptFolder, int linenr, int type) {
