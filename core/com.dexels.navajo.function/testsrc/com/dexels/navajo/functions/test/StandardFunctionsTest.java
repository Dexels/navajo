...
public class StandardFunctionsTest {
    ...
    private Navajo createTestNavajo() {
        ...
        Property p2 = NavajoFactory.getInstance().createProperty(doc, "Mies",
                Property.INTEGER_PROPERTY, "20", 10, "", "in");
        array1.addProperty(p2);

        Message single = NavajoFactory.getInstance().createMessage(doc, "Single");
        doc.addMessage(single);
        Property p3 = NavajoFactory.getInstance().createProperty(doc, "Selectie", "1", "", "in");
        p3.addSelection(NavajoFactory.getInstance().createSelection(doc, "key", "value", true));
        single.addProperty(p3);
        Property p4 = NavajoFactory.getInstance().createProperty(doc, "Vuur",
                Property.INTEGER_PROPERTY, "10", 10, "", "out");
        single.addProperty(p4);
    ...
    public void testXmlUnescapeWithBogusParameter() {
    ...
    public void testGetSelectedValue() {
        ...
        Navajo doc = createTestNavajo();
        fi.insertSelectionListOperand(
                doc.getProperty("/Single/Selectie").getAllSelectedSelections());
    ...
    public void testBase64Encode() {
        ...
        expected = expected.substring(0, expected.length() - 1); // strip newline
    ...
    public void testArraySelection() {
        ...
        fi.insertStringOperand("Mies");
        List<?> actual = (List<?>) fi.evaluateWithTypeChecking();
    ...
    public void testWeekDay() {
        ...
        wd.reset();
        wd.insertDateOperand(new Date(System.currentTimeMillis()));
        Assert.assertEquals(df.format(today).toUpperCase(), wd.evaluate().toString());
    }
