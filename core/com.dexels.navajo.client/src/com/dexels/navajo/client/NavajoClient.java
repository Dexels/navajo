...
public class NavajoClient implements ClientInterface, Serializable {
	...
	private long retryInterval = 1000; // default retry interval is 500
										// milliseconds
	private int retryAttempts = 4; // default four retry attempts
	private int switchServerAfterRetries = 10;
	/**
	 * If same as retry attempts, never switch between servers, while in retry
	 * attempt. FOR NOW THIS IS A SAFE VALUE CAUSE INTEGRITY WORKER DOES NOT YET
	 * WORKER OVER MULTIPLE SERVER INSTANCES!!!
	 */
	...
	public void setKeyStore(KeyStore keyStore) {
		if (socketFactory != null) {
			throw new RuntimeException(
					"Not yet implemented: combination of custom keystore and client certificates is not yet implemented");
		}
	...
	/**
	 * Sets the number of retries the NavajoClient should perform before giving
	 * up
	 * 
	 * @param attempts
	 *            int
	...
	public final Navajo doSimpleSend(Navajo n, String method,
			ConditionErrorHandler v, long expirationInterval)
			throws ClientException {
		if (v != null) {
	 ...
	 * @return Navajo
	 */
	private final Navajo doSimpleSend(Navajo out, String method,
	...
	/**
	 * Do a transation with the Navajo Server (name) using a Navajo Message
	 * Structure (TMS) compliant XML document.
	 * 
	 * @param name
	 *            String
	 * @param d
	...
	private final void generateConnectionError(Navajo n, int id,
		...
		try {
			Message conditionError = NavajoFactory.getInstance().createMessage(
					n, "ConditionErrors", Message.MSG_TYPE_ARRAY);
			n.addMessage(conditionError);
			Message conditionErrorElt = NavajoFactory.getInstance()
					.createMessage(n, "ConditionErrors");
			conditionError.addMessage(conditionErrorElt);
			Property p1 = NavajoFactory.getInstance().createProperty(n, "Id",
					Property.INTEGER_PROPERTY, id + "", 10, "Id",
					Property.DIR_OUT);
			Property p2 = NavajoFactory.getInstance().createProperty(n,
					"Description", Property.INTEGER_PROPERTY, description, 10,
					"Omschrijving", Property.DIR_OUT);
			Property p3 = NavajoFactory.getInstance().createProperty(n,
					"FailedExpression", Property.INTEGER_PROPERTY, "", 10,
					"FailedExpression", Property.DIR_OUT);
			Property p4 = NavajoFactory.getInstance().createProperty(n,
					"EvaluatedExpression", Property.INTEGER_PROPERTY, "", 10,
					"EvaluatedExpression", Property.DIR_OUT);
	...
	public final Navajo doSimpleSend(Navajo out, String server, String method,
		...
		synchronized (out) {
			...
			if (header == null) {
				header = NavajoFactory.getInstance().createHeader(out, method,
			...
			header.setHeaderAttribute("clientInfo", getSystemInfoProvider()
					.toString());
			...
			try {
				...
				if (protocol == HTTP_PROTOCOL) {
					...
					try {
						...
						logger.info("Generic IOException: " + uhe.getMessage()
								+ ". Retrying without compression...");
						Navajo n2 = NavajoFactory.getInstance().createNavajo();
						n = retryTransaction(server, out, false,
								allowPreparseProxy, retryAttempts,
								retryInterval, n2); // lees uit resourc
						if (n != null) {
							// logger.info("METHOD: "+method+" sourcehead: "+callingService+" sourceSource: "+out.getHeader().getHeaderAttribute("sourceScript")+" outRPCName: "+n.getHeader().getRPCName());
							if (callingService != null) {
								n.getHeader().setHeaderAttribute(
						...
						if (n != null && n.getHeader() != null) {
							n.getHeader().setHeaderAttribute("sourceScript",
									callingService);
							clientTime = (System.currentTimeMillis() - timeStamp);
							n.getHeader().setHeaderAttribute("clientTime",
									"" + clientTime);
							String tot = n.getHeader().getHeaderAttribute(
									"serverTime");
							String loadStr = n.getHeader().getHeaderAttribute(
									"cpuload");
							double load = -1.0;
							if (loadStr != null) {
								try {
							...
							Map<String, String> headerAttributes = n
									.getHeader().getHeaderAttributes();
							Map<String, String> pbd = new HashMap<String, String>(
									headerAttributes);
					...
					throw new ClientException(-1, -1, "Unknown protocol: "
							+ protocol);
				}
			} catch (Exception e) {
				logger.error("Error: ", e);
	...
	private final void processPiggybackData(Header header) {
		...
		synchronized (piggyBackData) {
			...
			for (Iterator<Map<String, String>> iter = piggyBackData.iterator(); iter
					.hasNext();) {
	...
	private final Navajo retryTransaction(String server, Navajo out,
			boolean useCompression, boolean allowPreparseProxy,
			int attemptsLeft, long interval, Navajo n) throws Exception {
		InputStream in = null;

		globalRetryCounter++;
		logger.info("------------> retrying transaction: " + server
				+ ", attempts left: " + attemptsLeft + " total retries: "
				+ globalRetryCounter);
		...
		if (pastAttempts >= (switchServerAfterRetries - 1)
				&& serverUrls.length > 1) {
			logger.info("Did: " + pastAttempts + " retries. Switching server");
			disabledServers.put(getCurrentHost(),
					new Long(System.currentTimeMillis()));
			logger.info("Disabled server: " + getCurrentHost() + " for "
					+ serverDisableTimeout + " millis.");
			switchServer(true);
		...
		try {
			...
			Navajo doc = doTransaction(server, out, useCompression,
					allowPreparseProxy);
			logger.info("It worked!  the inputstream is: " + in);
			return doc;
		} catch (java.net.UnknownHostException uhe) {
			generateConnectionError(n, 7777777,
			...
			if (attemptsLeft <= 0) {
				...
				generateConnectionError(
						n,
						4444,
						"Could not connect to server (network problem?) "
								+ uhe.getMessage());
			} else {
				return retryTransaction(server, out, useCompression,
						allowPreparseProxy, attemptsLeft, interval, n);
			}
		} catch (IOException uhe) {
			...
			if (attemptsLeft <= 0) {
				...
				attemptsLeft--;
				return retryTransaction(server, out, false, allowPreparseProxy,
						attemptsLeft, interval, n);
			}
		}
		return null;
	}
	...
	public void setServers(String[] servers) {
		...
		// logger.info("servers = " + servers[0] + ", loadBalancingMode = " +
		// loadBalancingMode);
	...
	public void setCurrentHost(String host) {
		for (int i = 0; i < serverUrls.length; i++) {
			if (serverUrls[i].equals(host)) {
				...
				logger.info("SET CURRENT SERVER TO: " + host + "("
						+ currentServerIndex + ")");
				break;
			}
		}
	...
	/**
	 * Switch the current server. If force flag is set. The server with the
	 * minimum load is ALWAYS set as the current server. For LBMODE_MANUAL and
	 * LBMODE_STATIC_MINLOAD the server is ONLY switched if force flag is set.
	...
	private final void switchServer(boolean force) {
		...
		for (int i = 0; i < serverUrls.length; i++) {
			if (serverLoads[i] < minload && serverLoads[i] != -1.0
					&& !disabledServers.containsKey(serverUrls[i])) { // If
																		// there
																		// is
																		// really
																		// a
		...
		logger.info("currentServer = " + serverUrls[currentServerIndex]
				+ " with load: " + serverLoads[currentServerIndex]);
	 ...
	 * @schedule defines a timestamp of the format: HH:mm:ss dd-MM-yyyy. If null
	 *           assume immediate execution.
	 * 
	 */
	@Override
	public Navajo doScheduledSend(Navajo out, String method, String schedule,
			String description, String clientId) throws ClientException {
		...
		if (h == null) {
			h = NavajoFactory.getInstance().createHeader(out, method, username,
					password, -1);
			out.addHeader(h);
		}
	...
	public Binary getArrayMessageReport(Message m, String[] propertyNames,
			String[] propertyTitles, int[] columnWidths, String format,
			String orientation, int[] margins) throws NavajoException {
		// Message m = in.getMessage(messagePath);
		if (m == null) {
			throw NavajoFactory.getInstance().createNavajoException(
					"Message not found. Can not run report.");
		}
		Navajo n = NavajoFactory.getInstance().createNavajo();
		Message cp = m.copy(n);
		Header h = NavajoFactory.getInstance().createHeader(n, "Irrelevant",
				"Irrelevant", "Irrelevant", -1);
		n.addHeader(h);
		h.setHeaderAttribute("sourceScript", "Irrelevant");
		n.addMessage(cp);
		Message repDef = NavajoFactory.getInstance().createMessage(n,
				"__ReportDefinition");
		n.addMessage(repDef);
		...
		Property sizeProp = NavajoFactory.getInstance().createProperty(n,
				"PropertySizes", Property.STRING_PROPERTY, sz.toString(), 0,
				"", Property.DIR_IN);
		...
		Property namesProp = NavajoFactory.getInstance().createProperty(n,
				"PropertyNames", Property.STRING_PROPERTY, propertyNamesString,
				0, "", Property.DIR_IN);
		...
		Property titlesProp = NavajoFactory.getInstance().createProperty(n,
				"PropertyTitles", Property.STRING_PROPERTY, sz.toString(), 0,
				"", Property.DIR_IN);
		repDef.addProperty(titlesProp);

		Property messagePathProp = NavajoFactory.getInstance().createProperty(
				n, "MessagePath", Property.STRING_PROPERTY, cp.getName(), 0,
				"", Property.DIR_IN);
		repDef.addProperty(messagePathProp);

		Property reportFormatProp = NavajoFactory.getInstance().createProperty(
				n, "OutputFormat", Property.STRING_PROPERTY, format, 0, "",
				Property.DIR_IN);
		repDef.addProperty(reportFormatProp);

		if (margins != null) {
			Property marginProperty = NavajoFactory.getInstance()
					.createProperty(
							n,
							"Margin",
							Property.STRING_PROPERTY,
							margins[0] + "," + margins[1] + "," + margins[2]
									+ "," + margins[3], 0, "", Property.DIR_IN);
			repDef.addProperty(marginProperty);
		}
		if (orientation != null) {
			Property orientationProperty = NavajoFactory.getInstance()
					.createProperty(n, "Orientation", Property.STRING_PROPERTY,
							orientation, 0, "", Property.DIR_IN);
			repDef.addProperty(orientationProperty);
		}

		try {
			Navajo result = NavajoClientFactory.getClient().doSimpleSend(n,
					"ProcessPrintTableBirt");
			Property data = result.getProperty("/Result/Data");
			if (data == null) {
				result.write(System.err);
				throw NavajoFactory.getInstance().createNavajoException(
						"No report property found.");
			}
