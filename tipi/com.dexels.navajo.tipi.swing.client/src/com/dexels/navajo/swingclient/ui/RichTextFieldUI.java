...
public class RichTextFieldUI extends BasicTextFieldUI {
	...
	protected Rectangle getVisibleEditorRect() {
		...
		if (parent != null) {
			return new Rectangle(parent.x + text_inset, parent.y, parent.width
					- 2 * text_inset, parent.height);
		} else {
	...
	protected void paintSafely(Graphics g) {
		...
		if (!me.hasFocus() && enabled) {
			...
			g2.setPaint(new GradientPaint(0, 0, Color.black, 0, bounds.height,
					Color.white));
			g2.drawRoundRect(0, 0, bounds.width - borderWidth, bounds.height
					- borderWidth, arc, arc);
		} else if (enabled) {
