...
public class DefaultDayRenderer extends JPanel implements DayRendererInterface {
	...
	public void setDay(Day d) {
		...
		if (img_url != null) {
			myBackgroundImage = (new ImageIcon(
					DefaultDayRenderer.class.getResource(img_url))).getImage();
		} else {
	...
	public void paintComponent(Graphics g) {
		...
		if (!myLabel.equals("")) {
			...
			if (nrOfAttributes > 0) {
				dayPaint = new GradientPaint(img.getWidth() / 2, 0,
						CalendarConstants
								.getColor(CalendarConstants.ATTRIBUTED_COLOR),
						img.getWidth() / 2, getHeight(),
						CalendarConstants
								.getColor(CalendarConstants.BACKGROUND_COLOR));
			} else {
				dayPaint = new GradientPaint(
						img.getWidth() / 2,
						0,
						CalendarConstants.getColor(CalendarConstants.DAY_COLOR),
						img.getWidth() / 2, getHeight(), CalendarConstants
								.getColor(CalendarConstants.BACKGROUND_COLOR));
			}
			offScreen.setPaint(dayPaint);
			offScreen.fillRect(0, 0, img.getWidth(), img.getHeight());
			if (myBackgroundImage != null) {
				offScreen.drawImage(myBackgroundImage, img.getWidth()
						- myBackgroundImage.getWidth(this) - 2, 0, this);
			}
			...
			if (isSelected) {
				...
				dayPaint = new GradientPaint(img.getWidth() / 2, 0,
						CalendarConstants
								.getColor(CalendarConstants.SELECTION_COLOR),
						img.getWidth() / 2, getHeight(),
						CalendarConstants
								.getColor(CalendarConstants.BACKGROUND_COLOR));
				offScreen.setPaint(dayPaint);
			...
			offScreen.setColor(CalendarConstants
					.getColor(CalendarConstants.DAY_BORDER_COLOR));
			offScreen.drawRect(0, 0, img.getWidth() - 1, img.getHeight() - 1);

			if (isAnchor) {
				offScreen.setColor(CalendarConstants
						.getColor(CalendarConstants.DAY_ANCHOR_COLOR));
				offScreen.drawRect(0, 0, img.getWidth() - 2,
						img.getHeight() - 2);
			}

			offScreen.setColor(CalendarConstants
					.getColor(CalendarConstants.DAY_FONT_COLOR));
			...
			GradientPaint dayPaint = new GradientPaint(img.getWidth() / 2, 0,
					CalendarConstants.getColor(CalendarConstants.NONDAY_COLOR),
					img.getWidth() / 2, getHeight(),
					CalendarConstants
							.getColor(CalendarConstants.BACKGROUND_COLOR));
			offScreen.setPaint(dayPaint);
