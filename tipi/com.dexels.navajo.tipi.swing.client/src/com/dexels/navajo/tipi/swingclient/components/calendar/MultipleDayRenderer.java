...
public class MultipleDayRenderer extends JPanel {
		...
		if (!myLabel.equals("")) {
			...
			if (isSelected) {
				dayPaint = new GradientPaint(img.getWidth() / 2, 0,
						Color.decode("#7A9BF8"), img.getWidth() / 2, 70,
						Color.white);
			} else if (myAttributeCount > 1) {
				dayPaint = new GradientPaint(img.getWidth() / 2, 0,
						Color.decode("#77FF77"), img.getWidth() / 2, 70,
						Color.white);
			} else {
				dayPaint = new GradientPaint(img.getWidth() / 2, 0,
						Color.decode("#EEEEEE"), img.getWidth() / 2, 70,
						Color.white);
			}
			...
			GradientPaint dayPaint = new GradientPaint(img.getWidth() / 2, 0,
					Color.darkGray, img.getWidth() / 2, 70, Color.black);
			offScreen.setPaint(dayPaint);
			offScreen.fillRect(0, 0, img.getWidth(), img.getHeight());
			offScreen.setColor(Color.black);
			offScreen.drawRect(0, 0, img.getWidth() - 1, img.getHeight() - 1);
