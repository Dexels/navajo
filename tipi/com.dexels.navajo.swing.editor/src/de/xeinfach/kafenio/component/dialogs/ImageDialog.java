...
public class ImageDialog extends JDialog implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 5393669680496702590L;
	 ...
	 *            image paths.
	 */
	public ImageDialog(KafenioPanel newKafenio, Vector imageNames,
			Vector imagePaths) {
		super(newKafenio.getFrame(), newKafenio
				.getTranslation("InsertServerImageDialogTitle"), true);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals(ACTION_INSERT)) {
			...
			if (sm.isSelectionEmpty()) {
				SimpleInfoDialog sidAbout = new SimpleInfoDialog(kafenio,
				...
				updatePreview();
				selectedImage = controls.getHtmlImgString(
						imageAltText.getText(), previewImage);
				hide();
			}
	...
	private void initComponents() {
		...
		JSplitPane splitterPanel = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT,
				true, imageScrollPane, previewViewport);
		JPanel centerPanel = new JPanel(new BorderLayout());
		centerPanel.add(splitterPanel, BorderLayout.CENTER);
		centerPanel
				.setBorder(BorderFactory
						.createTitledBorder(getString("InsertImageDialogServerImagesBorder")));

		/* creating the panel for alternative text */
		JLabel imageAltTextLabel = new JLabel(
				getString("InsertImageDialogAlternateTextField"),
				SwingConstants.LEFT);
		...
		JButton insertButton = new JButton(
				getString("InsertImageDialogInsertButton"));
	...
	private void loadSelectedPreviewImage() {
		...
		if (!sm.isSelectionEmpty()) {
			...
			try {
				...
				controls.setNewImage(imageIcon.getIconWidth(),
	...
	/**
	 * This function is called by dialog executee to know what image was
	 * selected.
	 * 
	...
	/**
	 * updates the preview image using the values from ImageDialogControls
	 * object.
	 */
	public void updatePreview() {
		try {
			htmlDoc = getDefaultDocument();
			previewPane.setDocument(htmlDoc);
			String imgString = controls.getHtmlImgString(
					imageAltText.getText(), previewImage);
			htmlKit.insertHTML(htmlDoc, 0, imgString, 0, 0, HTML.Tag.IMG);
			validate();
		} catch (Exception e) {
			log.error("an error ocurred while updating the preview image: "
					+ e.fillInStackTrace());
		}
	}
	...
	private ExtendedHTMLDocument getDefaultDocument() {
		try {
			return (ExtendedHTMLDocument) (htmlKit
					.createDefaultDocument(new URL(kafenio.getConfig()
							.getCodeBase())));
		} catch (Exception e) {
