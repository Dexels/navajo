...
/**
 * Description: HTMLUtilities - Special Utility Functions For Kafenio Copyright
 * (C) 2003 Rafael Cieplinski, modified by Howard Kistler
 * 
 * @author Copyright (C) 2003 Rafael Cieplinski, modified by Howard Kistler,
 *         Karsten Pawlik
 */
public class HTMLUtilities {
	 ...
	 * 
	 * @param newParent
	 *            a reference to an instance of the KafenioPanel class.
	 */
	...
	public void insertListElement(String content) {
		...
		parentKafenioPanel.getExtendedHtmlDoc().replaceAttributes(element, sa,
				HTML.Tag.LI);
		...
		newHtmlString.append("</li>");
		newHtmlString
				.append(source.substring(positions[3] + 1, source.length()));
	...
	/**
	 * Method to remove the given element using stringmanipulation in the
	 * source-pane.<BR>
	...
	/**
	 * Method to check if the given tag is in the hierarchy above the current
	 * position or not.
	 * 
	 * @param tag
	...
	public boolean checkParentsTag(HTML.Tag tag) {
		Element e = parentKafenioPanel.getExtendedHtmlDoc()
				.getParagraphElement(parentKafenioPanel.getCaretPosition());
	...
	public Element getListItemParent() {
		...
		Element elementSearch = parentKafenioPanel.getExtendedHtmlDoc()
				.getCharacterElement(parentKafenioPanel.getCaretPosition());
		do {
	 ...
	 * @return Diese Methode entfernt Attribute aus dem SimpleAttributeSet,
	 *         gem�� den �bergebenen Werten, und gibt das Ergebnis als
	 *         SimpleAttributeSet zur�ck
	 */
	public SimpleAttributeSet removeAttribute(SimpleAttributeSet sourceAS,
		...
		try {
			...
			while (removeEn.hasMoreElements()) {
				...
				removeValues[j] = removeAS.getAttribute(removeKeys[j])
						.toString();
				j++;
			}
			...
			for (int countSource = 0; countSource < sourceKeys.length; countSource++) {
				...
				if (!hit) {
					result.addAttribute(sourceKeys[countSource].toString(),
	...
	public void delete() throws BadLocationException, IOException {
		...
		htmlDoc.insertString(selEnd + posStrings[0].length(), posStrings[1],
				null);
		...
		htmlString += jtpSource.getText().substring(
				start + posStrings[0].length(), end);
		htmlString += jtpSource.getText().substring(
	...
	private String getAllTableTags(String source) throws BadLocationException,
		...
		do {
			...
			if (caret != -1) {
				result.append(source.substring(caret,
						source.indexOf(">", caret) + 1));
			}
		} while (caret != -1);
