...
public class RelativeImageView extends View implements ImageObserver,
	...
	private static LeanLogger log = new LeanLogger("RelativeImageView.class");
		...
		try {
			...
			if (isURL()) {
				...
				try {
					...
					log.warn("exception thrown while processing src path. "
							+ e.fillInStackTrace());
					src = (String) fElement.getAttributes().getAttribute(
							HTML.Attribute.SRC);
				}
	...
	private String processSrcPath(String src) {
		...
		if (doc != null) {
			String pv = (String) (doc
					.getProperty("de.xeinfach.kafenio.docsource"));
	...
	public float getVerticalAlignment() {
		...
		if (align != null) {
			...
			if (align.equals(TOP) || align.equals(TEXTTOP)) {
				return 0.0f;
			} else if (align.equals(RelativeImageView.CENTER)
					|| align.equals(MIDDLE) || align.equals(ABSMIDDLE)) {
				return 0.5f;
			}
		}
	...
	public void paint(Graphics g, Shape a) {
		...
		if (!hasPixels(this)) {
			...
			Icon icon = ((fImage == null) ? sMissingImageIcon
					: sPendingImageIcon);
			if (icon != null) {
	...
	protected void resize(int width, int height) {
		...
		((StyledDocument) getDocument()).setCharacterAttributes(
				fElement.getStartOffset(), fElement.getEndOffset(), myAttr,
				false);
	}
	...
	private Icon makeIcon(final String gifFile) throws IOException {
		 ...
		 */
		InputStream resource = RelativeImageView.class
