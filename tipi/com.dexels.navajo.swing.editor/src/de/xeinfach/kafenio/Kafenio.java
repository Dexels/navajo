 ...
 */
public class Kafenio extends JFrame implements WindowListener,
		KafenioContainerInterface {

	/**
	 * 
	 */
	private static final long serialVersionUID = -3305820524738428678L;
	...
	public Kafenio(KafenioPanelConfigurationInterface iConfiguration) {
		...
		try {
			// What follows is more complicated than necessary in order to
			// demonstrate
			// plugin-like employment of Kafenio. Easy, non plugin way, would
			// be: kafenioPanel = new KafenioPanel(config);
			Class kafenioPanelClass = Class
					.forName("de.xeinfach.kafenio.KafenioPanel");
			Class[] constructorParameterTyping = { KafenioPanelConfigurationInterface.class };
			Constructor kafenioPanelContructor = kafenioPanelClass
					.getConstructor(constructorParameterTyping);
			kafenioPanel = (KafenioPanelInterface) kafenioPanelContructor
	...
	public static void main(String[] args) {
		...
		try {
			newConfig = (KafenioPanelConfigurationInterface) Class.forName(
					"de.xeinfach.kafenio.KafenioPanelConfiguration")
					.newInstance();
		} catch (Exception ex) {
		}
		...
		if (args.length > 0) {
			...
			for (int i = 0; i < args.length; i++) {
				if (args[i].equals("-t")) {
					...
					newConfig
							.setDocument(args[i].substring(2, args[i].length()));
				} else if (args[i].startsWith("-c")) {
					newConfig.setStyleSheet(args[i].substring(2,
							args[i].length()));
				} else if (args[i].startsWith("-r")) {
					newConfig.setRawDocument(args[i].substring(2,
							args[i].length()));
				} else if (args[i].startsWith("-u")) {
					try {
						newConfig.setUrlStyleSheet(new URL(args[i].substring(2,
								args[i].length())));
					} catch (Throwable e) {
						log.warn("Exception caught while trying to setURLStylesheet: "
								+ e.fillInStackTrace());
					}
				} else if (args[i].startsWith("-l")) {
					if (args[i].indexOf('_') > -1) {
						newConfig.setLanguage(args[i].substring(2,
								args[i].indexOf('_')));
						newConfig.setCountry(args[i].substring(
								args[i].indexOf('_') + 1, args[i].length()));
					}
	...
	private void updateTitle() {
		this.setTitle(kafenioPanel.getAppName()
