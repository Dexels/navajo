...
public class KafenioToolBar {
	...
	public static final String KEY_TOOL_SAVEAS = "saveas";
	...
	/**
	 * method that returns a JToolBar object containing the buttons specified in
	 * toolbarItemList. the button names are separated by spaces.
	 * 
	 * @param toolbarItemList
	 *            space-separated list of buttons names.
	 * @param show
	 *            if show is false, the method returns null.
	...
	private Hashtable createItems() {
		...
		items.put(KEY_TOOL_NEW,
				createButton("New", "NewDocument", "newdoc", getParent(), null));
		items.put(
				KEY_TOOL_OPEN,
				createButton("Open", "OpenDocument", "openhtml", getParent(),
						null));
		items.put(KEY_TOOL_SAVE,
				createButton("Save", "SaveDocument", "save", getParent(), null));
		items.put(
				KEY_TOOL_SAVEAS,
				createButton("Save", "SaveDocument", "saveas", getParent(),
						null));
		items.put(KEY_TOOL_SEP, createSeparator());
		items.put(
				KEY_TOOL_CUT,
				createButton("Cut", "Cut", null, null,
						new DefaultEditorKit.CutAction()));
		items.put(
				KEY_TOOL_COPY,
				createButton("Copy", "Copy", null, null,
						new DefaultEditorKit.CopyAction()));
		items.put(
				KEY_TOOL_PASTE,
				createButton("Paste", "Paste", null, null,
						new DefaultEditorKit.PasteAction()));
		items.put(
				KEY_TOOL_BOLD,
				createButton("Bold", "FontBold", null, null, getActionList()
						.getActionFontBold()));
		items.put(
				KEY_TOOL_ITALIC,
				createButton("Italic", "FontItalic", null, null,
						getActionList().getActionFontItalic()));
		items.put(
				KEY_TOOL_UNDERLINE,
				createButton("Underline", "FontUnderline", null, null,
						getActionList().getActionFontUnderline()));
		items.put(
				KEY_TOOL_STRIKE,
				createButton("Strike", "FontStrike", null, null,
						getActionList().getActionFontStrike()));
		items.put(
				KEY_TOOL_SUPER,
				createButton("Super", "FontSuperscript", null, null,
						getActionList().getActionFontSuperscript()));
		items.put(
				KEY_TOOL_SUB,
				createButton("Sub", "FontSubscript", null, null,
						getActionList().getActionFontSubscript()));
		items.put(
				KEY_TOOL_ULIST,
				createButton("UList", "ListUnordered", null, null,
						getActionList().getActionListUnordered()));
		items.put(
				KEY_TOOL_OLIST,
				createButton("OList", "ListOrdered", null, null,
						getActionList().getActionListOrdered()));
		items.put(
				KEY_TOOL_CLEAR,
				createButton("ClearFormat", "FormatClear", null, null,
						getActionList().getActionClearFormat()));
		items.put(
				KEY_TOOL_CHARACTER,
				createButton("Character", "InsertCharacterDialogTitle", null,
						null, getActionList().getActionInsertCharacter()));
		items.put(
				KEY_TOOL_ALIGNLEFT,
				createButton("AlignLeft", "AlignLeft", null, null,
						getActionList().getActionAlignLeft()));
		items.put(
				KEY_TOOL_ALIGNCENTER,
				createButton("AlignCenter", "AlignCenter", null, null,
						getActionList().getActionAlignCenter()));
		items.put(
				KEY_TOOL_ALIGNRIGHT,
				createButton("AlignRight", "AlignRight", null, null,
						getActionList().getActionAlignRight()));
		items.put(
				KEY_TOOL_ALIGNJUSTIFIED,
				createButton("AlignJustified", "AlignJustified", null, null,
						getActionList().getActionAlignJustified()));
		if (config.isApplet()) {
			log.debug("insert server image");
			items.put(
					KEY_TOOL_IMAGE,
					createButton("InsertImage", "InsertServerImage", null,
							null, getActionList().getActionInsertServerImage()));
		} else {
			log.debug("insert local image");
			items.put(
					KEY_TOOL_IMAGE,
					createButton("InsertImage", "InsertLocalImage",
							"insertlocalimage", getParent(), null));
		}

		items.put(
				KEY_TOOL_UNDO,
				createButton("Undo", "Undo", null, getParent(), getParent()
						.getUndoAction()));
		items.put(
				KEY_TOOL_REDO,
				createButton("Redo", "Redo", null, getParent(), getParent()
						.getRedoAction()));
		items.put(
				KEY_TOOL_INDENTLEFT,
				createButton("IndentLeft", "IndentLeft", null, null,
						getActionList().getActionIndentLeft()));
		items.put(
				KEY_TOOL_INDENTRIGHT,
				createButton("IndentRight", "IndentRight", null, null,
						getActionList().getActionIndentRight()));
		items.put(
				KEY_TOOL_ANCHOR,
				createButton("Anchor", "ToolAnchor", null, null,
						getActionList().getActionInsertHyperlink()));
		items.put(KEY_TOOL_FIND,
				createButton("Find", "SearchFind", "find", getParent(), null));
		items.put(
				KEY_TOOL_COLOR,
				createButton("Color", "CustomColor", null, null,
						getActionList().getActionFontCustomColor()));
		items.put(
				KEY_TOOL_TABLE,
				createButton("Table", "InsertTable", "inserttable",
						getParent(), null));
		items.put(
				KEY_TOOL_SOURCE,
				createButton("Source", "ViewSource", "viewsource", getParent(),
						null));
		items.put(
				KEY_TOOL_SAVECONTENT,
				createButton("SaveContent", "SaveDocument", "savecontent",
						getParent(), null));
		items.put(KEY_TOOL_CONFIRMCONTENT, createButton("SaveContent", // Although
																		// ConfirmContent,
																		// use
																		// the
																		// icon
																		// of
																		// SaveContent
				"ConfirmContent", "confirmcontent", getParent(), null));
		items.put(
				KEY_TOOL_DETACHFRAME,
				createButton("DetachFrame", "DetachFrame", "detachframe",
						getParent(), null));
	 ...
	 * 
	 * @param icon
	 *            the icon string without path and the trailing "HK.gif" at the
	 *            end.
	 * @param labelText
	 *            the id of the tooltip text to translate
	 * @param actionCommand
	 *            an action command (can be null if action is specified)
	 * @param listener
	 *            an action listener (can be null if action is specified)
	 * @param action
	 *            an action (can be null if actionCommand and listener are
	 *            specified.)
	 * @return returns a new JButtonNoFocus Object.
	 */
	private JButtonNoFocus createButton(String icon, String labelText,
			String actionCommand, ActionListener listener, Action action) {
		...
		if (action != null) {
			...
			newNoFocusButton = new JButtonNoFocus(
					(getParent().getKafenioIcon(icon)));
