...
public class KafenioPanel extends JPanel implements ActionListener,
    ...
    public void keyTyped(KeyEvent ke) {
        ...
        //} else if (ke.getKeyChar() == KeyEvent.VK_TAB) {
            ...
            try {
                if (htmlUtils.checkParentsTag(HTML.Tag.UL) | htmlUtils.checkParentsTag(HTML.Tag.OL)) {
                    if (ke.isShiftDown()) {
                        log.debug("shift-enter pressed inside list item.");
                    	getKafenioActions().getActionAddNewLine().actionPerformed(new ActionEvent(ke.getSource(), 10, ""));
                    } else {
                    ...
                    if (!ke.isShiftDown()) {
                    	getKafenioActions().getActionAddNewLine().actionPerformed(new ActionEvent(ke.getSource(), 10, ""));
                        //getKafenioActions().getActionAddParagraph().actionPerformed(new ActionEvent(ke.getSource(), 10, ""));
                    }
                }
            } catch (BadLocationException ble) {
                log.error("BadLocationException in keyTyped method: " + ble.fillInStackTrace());
        ...
        } else if (ke.getKeyChar() == KeyEvent.VK_ESCAPE && getConfig().getProperty("escapeCloses").equals("true")) {
