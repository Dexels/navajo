...
/**
 * The Action for inserting an image from the server. Pretty simple and
 * straightforward, all exceptions handled.
 * 
...
public class ServerImageAction extends StyledEditorKit.StyledTextAction {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 76908081937342584L;
	 ...
	 * 
	 * @param newKafenio
	 *            the parent KafenioPanel.
	 */
	public ServerImageAction(KafenioPanel newKafenio) {
		super(newKafenio.getTranslation("InsertServerImage")
				+ KafenioMenuBar.DOTS);
		...
		try {
			if (selectedImage != null && !selectedImage.equals("")) {
				kafenio.getExtendedHtmlKit().insertHTML(
						kafenio.getExtendedHtmlDoc(), caretPos, selectedImage,
						0, 0, HTML.Tag.IMG);
				kafenio.setCaretPosition(caretPos + 1);
			}
		} catch (BadLocationException ble) {
			log.error("BadLocationException in ServerImageAction: "
					+ ble.fillInStackTrace());
		} catch (IOException ioe) {
			log.error("IOException in ServerImageAction: "
