 ...
 * @version 1.0
 */
public class TipiSwingGridBagConstraints extends GridBagConstraints {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1649681473224995726L;
	private String[] myConstraints = new String[14];

	public TipiSwingGridBagConstraints() {
	}

	public TipiSwingGridBagConstraints(int x, int y, int w, int h, double wx,
			double wy, int anchor, int fill, Insets insets, int padx, int pady) {
	...
	public void parse(String cs) {
		...
		if (tokenCount != 14) {
			...
			insets = new Insets(Integer.parseInt(myConstraints[8]),
					Integer.parseInt(myConstraints[9]),
					Integer.parseInt(myConstraints[10]),
					Integer.parseInt(myConstraints[11]));
