...
import com.dexels.navajo.tipi.swing.colorpicker.plaf.ColorPickerSliderUI;

/**
 * <p>
 * This is a panel that offers a robust set of controls to pick a color.
 * <P>
 * This was originally intended to replace the <code>JColorChooser</code>. To use this class to create a color choosing dialog, simply call: <BR>
 * <code>ColorPicker.showDialog(frame, originalColor);</code>
 * <P>
 * Here is a screenshot of the dialog that call will invoke: <br>
 * <IMG SRC="https://javagraphics.java.net/resources/colorpicker.png" alt="ColorPicker Screenshot">
 * <p>
 * However this does not have to invoked as a black-box color dialog. This class is simply a panel, and you can customize and resize it for other looks. For
 ...
 * <IMG SRC="https://javagraphics.java.net/resources/colorpicker3.png" alt="ColorPicker Small Screenshot">
 * <P>
 * This will create a miniature color picker that still lets the user choose from every available color, but it does not include all the buttons and numeric
 * controls on the right side of the panel. This might be ideal if you are working with limited space, or non-power-users who don't need the RGB values of a
 * color.
 * <P>
 * To listen to color changes to this panel, you can add a <code>PropertyChangeListener</code> listening for changes to the <code>SELECTED_COLOR_PROPERTY</code>
 * . This will be triggered only when the RGB value of the selected color changes.
 * <P>
 * To listen to opacity changes to this panel, use a <code>PropertyChangeListener</code> listening for changes to the <code>OPACITY_PROPERTY</code>.
 * 
...
public class ColorPicker extends JPanel {
    ...
    protected static ResourceBundle strings = ResourceBundle.getBundle("resources/ColorPicker");
     ...
     * 
     * @param owner the dialog this new dialog belongs to. This must be a Frame or a Dialog. Java 1.6 supports Windows here, but this package is
     *            designed/compiled to work in Java 1.4, so an <code>IllegalArgumentException</code> will be thrown if this component is a <code>Window</code>.
     ...
     */
    public static Color showDialog(Component owner, Color originalColor) {
     ...
     * @return the <code>Color</code> the user chooses, or <code>null</code> if the user cancels the dialog.
     */
    public static Color showDialog(Component owner, Color originalColor, GradientPaint bgpaint) {
        return showDialog(owner, null, originalColor, false, bgpaint);
    }
     ...
     * @return the <code>Color</code> the user chooses, or <code>null</code> if the user cancels the dialog.
     */
    public static Color showDialog(Component owner, String title, Color originalColor, boolean includeOpacity, GradientPaint bgpaint) {
        ColorPickerDialog d;
        if (owner instanceof JFrame || owner == null) {
            d = new ColorPickerDialog((JFrame) owner, originalColor, includeOpacity);
        } else if (owner instanceof Dialog) {
            d = new ColorPickerDialog((Dialog) owner, originalColor, includeOpacity);
        } else {
            throw new IllegalArgumentException("the owner (" + owner.getClass().getName() + ") must be a javax.swing.JFrame or a java.awt.Dialog");
        }

        d.setTitle(title == null ? strings.getObject("ColorPickerDialogTitle").toString() : title);
        d.pack();
        d.setBackgroundPaint(bgpaint);
        d.setVisible(true);
     ...
     * <P>
     * (Events are only created when then RGB values of the color change. This means, for example, that the change from HSB(0,0,0) to HSB(.4,0,0) will
     * <i>not</i> generate events, because when the brightness stays zero the RGB color remains (0,0,0). So although the hue moved around, the color is still
    ...
    /**
     * <code>PropertyChangeEvents</code> will be triggered for this property when <code>setModeControlsVisible()</code> is called.
     */
    ...
    /**
     * <code>PropertyChangeEvents</code> will be triggered when the mode changes. (That is, when the wheel switches from HUE, SAT, BRI, RED, GREEN, or BLUE
     * modes.)
     */
    ...
    class HexDocumentListener implements DocumentListener {
        ...
        public void changedUpdate(DocumentEvent e) {
            ...
            /*
             * If we don't have 6 characters, then use a delay. If, after a second or two, the user has just stopped typing: then we can try to make sense of
             * what they input even if its incomplete.
             */
            boolean delay = false;
            if (s.length() < 6) {
                delay = true;
            ...
            try {
                ...
                if (delay) {
                    ...
                    /*
                     * Be sure to invoke this separately, otherwise we'll risk getting a "attempt to mutate in notification". (
         ...
         * @return an uppercase version of <code>s</code> that only includes hexadecimal characters and is not longer than <code>charLimit</code>.
         */
    ...
    /**
     * Used to indicate when we're internally adjusting the value of the spinners. If this equals zero, then incoming events are triggered by the user and must
     * be processed. If this is not equal to zero, then incoming events are triggered by another method that's already responding to the user's actions.
     */
    private int adjustingSpinners = 0;

    /**
     * Used to indicate when we're internally adjusting the value of the slider. If this equals zero, then incoming events are triggered by the user and must be
     * processed. If this is not equal to zero, then incoming events are triggered by another method that's already responding to the user's actions.
     */
    private int adjustingSlider = 0;

    /**
     * Used to indicate when we're internally adjusting the selected color of the ColorPanel. If this equals zero, then incoming events are triggered by the
     * user and must be processed. If this is not equal to zero, then incoming events are triggered by another method that's already responding to the user's
     * actions.
     */
    private int adjustingColorPanel = 0;

    /**
     * Used to indicate when we're internally adjusting the value of the hex field. If this equals zero, then incoming events are triggered by the user and must
     * be processed. If this is not equal to zero, then incoming events are triggered by another method that's already responding to the user's actions.
     */
    private int adjustingHexField = 0;

    /**
     * Used to indicate when we're internally adjusting the value of the opacity. If this equals zero, then incoming events are triggered by the user and must
     * be processed. If this is not equal to zero, then incoming events are triggered by another method that's already responding to the user's actions.
     */
     ...
     * 
     * @param showExpertControls the labels/spinners/buttons on the right side of a <code>ColorPicker</code> are optional. This boolean will control whether
    ...
    public ColorPicker(boolean showExpertControls, boolean includeOpacity) {
        ...
        Option[] optionsArray = new Option[] { red, green, blue };
     ...
     * <P>
     * Note this lives inside the "expert controls", so if <code>setExpertControlsVisible(false)</code> has been called, then calling this method makes no
    ...
    /**
     * The labels/spinners/buttons on the right side of a <code>ColorPicker</code> are optional. This method will control whether they are shown or not.
     * <P>
     * It may be that your users will never need or want numeric control when they choose their colors, so hiding this may simplify your interface.
     * 
    ...
    /**
     * Sets the mode of this <code>ColorPicker</code>. This is especially useful if this picker is in non-expert mode, so the radio buttons are not visible for
     ...
     * <P>
     * Note these live inside the "expert controls", so if <code>setExpertControlsVisible(false)</code> has been called, then these will never be visible.
     * 
     * @param b
     */
     ...
     *         This will return <code>HUE</code>, <code>SAT</code>, <code>BRI</code>, <code>RED</code>, <code>GREEN</code>, or <code>BLUE</code>.
     *         <P>
    ...
    /**
     * Sets the current color of this <code>ColorPicker</code>. This method simply calls <code>setRGB()</code> and <code>setOpacity()</code>.
     * 
     ...
     * <P>
     * (This returns the panel this <code>ColorPicker</code> uses, so if you put it in another container, it will be removed from this <code>ColorPicker</code>
     * .)
     * 
     ...
     * <P>
     * Note these live inside the "expert controls", so if <code>setExpertControlsVisible(false)</code> has been called, then calling this method makes no
     ...
     * <P>
     * Note the alpha spinner live inside the "expert controls", so if <code>setExpertControlsVisible(false)</code> has been called, then this method does not
     * affect that spinner. However, the opacity slider is <i>not</i> affected by the visibility of the export controls.
     * 
     * @param b
     */
    ...
    class Option {
        ...
        public Option(String text, int max) {
            ...
            /*
             * this tries out Tim Boudreaux's new slider UI. It's a good UI, but I think for the ColorPicker the numeric controls are more useful. That is:
             * users who want click-and-drag control to choose their colors don't need any of these Option objects at all; only power users who may have
             * specific RGB values in mind will use these controls: and when they do limiting them to a slider is unnecessary. That's my current position... of
             * course it may not be true in the real world... :)
             */
