 ...
 */
package com.dexels.navajo.tipi.swing.colorpicker.plaf;
...
/**
 * This listens for arrow keys and shifts the keyboard focus accordingly. So if you press the left arrow key, the component to the left of the source component
 * requests the focus.
 * <P>
 * This scans for the first available component whose <code>isFocusable()</code> method returns <code>true</code>. If no such component is found: nothing
 * happens.
 */
public class FocusArrowListener extends KeyAdapter {
     ...
     * @return true if another component requested the focus as a result of this method. This may return false if no suitable component was found to shift focus
     *         to. (If you press the right arrow key on the right-most component, for example.)
     */
     ...
     * <P>
     * My first implementation involved of this concept simply involved asking JCompnonents if they were focusable, but in the <code>FilledButtonTest</code>
     * this resulted in shifting focus to the ContentPane. Although it is technically focusable: if I used the tab key I did <i>not</i> get this result. So I
     * studied the inner workings for Component.transferFocus() and ended up with a method that involved calls to <code>getFocusCycleRootAncestor()</code>, and
