...
import com.dexels.navajo.tipi.swing.colorpicker.plaf.PlafPaintUtils;

/**
 * This is the large graphic element in the <code>ColorPicker</code> that depicts a wide range of colors.
 * <P>
 * This panel can operate in 6 different modes. In each mode a different property is held constant: hue, saturation, brightness, red, green, or blue. (Each
 * property is identified with a constant in the <code>ColorPicker</code> class, such as: <code>ColorPicker.HUE</code> or <code>ColorPicker.GREEN</code>.)
 * <P>
 * In saturation and brightness mode, a wheel is used. Although it doesn't use as many pixels as a square does: it is a very aesthetic model since the hue can
 * wrap around in a complete circle. (Also, on top of looks, this is how most people learn to think the color spectrum, so it has that advantage, too). In all
 * other modes a square is used.
 * <P>
 * The user can click in this panel to select a new color. The selected color is highlighted with a circle drawn around it. Also once this component has the
 * keyboard focus, the user can use the arrow keys to traverse the available colors.
 * <P>
 * Note this component is public and exists independently of the <code>ColorPicker</code> class. The only way this class is dependent on the
...
public class ColorPickerPanel extends JPanel {
    ...
    /*
     * Floats from [0,1]. They must be kept distinct, because when you convert them to RGB coordinates HSB(0,0,0) and HSB (.5,0,0) and then convert them back to
     * HSB coordinates, the hue always shifts back to zero.
     */
    ...
    MouseInputListener mouseListener = new MouseInputAdapter() {
        ...
        public void mousePressed(MouseEvent e) {
            requestFocus();
            Point p = e.getPoint();
            if (mode == ColorPicker.BRI || mode == ColorPicker.SAT || mode == ColorPicker.HUE) {
                float[] hsb = getHSB(p);
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (dx != 0 || dy != 0) {
                int size = Math.min(MAX_SIZE,
                        Math.min(getWidth() - imagePadding.left - imagePadding.right, getHeight() - imagePadding.top - imagePadding.bottom));

                int offsetX = getWidth() / 2 - size / 2;
                int offsetY = getHeight() / 2 - size / 2;
                mouseListener.mousePressed(new MouseEvent(ColorPickerPanel.this, MouseEvent.MOUSE_PRESSED, System.currentTimeMillis(), 0,
     ...
     * 
     * @param mode This must be one of the following constants from the <code>ColorPicker</code> class: <code>HUE</code>, <code>SAT</code>, <code>BRI</code>,
     *            <code>RED</code>, <code>GREEN</code>, or <code>BLUE</code>
     */
    public void setMode(int mode) {
        if (!(mode == ColorPicker.HUE || mode == ColorPicker.SAT || mode == ColorPicker.BRI || mode == ColorPicker.RED || mode == ColorPicker.GREEN
     ...
     * <P>
     * If this panel is in HUE, SAT, or BRI mode, then this method converts these values to HSB coordinates and calls <code>setHSB</code>.
     * <P>
     ...
     * Sets the selected color of this panel.
     * <P>
     * If this panel is in RED, GREEN, or BLUE mode, then this method converts these values to RGB coordinates and calls <code>setRGB</code>.
     * <P>
     * This method may regenerate the graphic if necessary.
     * 
