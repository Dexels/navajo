...
//		application/x-www-form-urlencoded
	...
	public void addRepositoryInstance(RepositoryInstance a,Map<String,Object> settings) {
...
//		writer.writeValue(System.err, node);
		...
		File applicationFolder = new File(repositoryManager.getRepositoryFolder(),applicationName);
		...
		Runnable r = new Runnable() {
			...
			@Override
			public void run() {
				final GitRepositoryInstance application = findApplication(url,
						branch);
				try {
					if (application !=null) {
						try {
						...
						logger.warn("No repository found from url: "+url+" with branch: "+branch);
					}
	...
	private GitRepositoryInstance findApplication(String gitUrl, String branch) {
		for (Map.Entry<RepositoryInstance, Map<String,Object>> e : repositorySettings.entrySet()) {
			...
			if(s!=null) {
				if(gitUrl.equals(url)) {
					if(branch.equals(s.get("branch"))) {
						logger.info("Found application for repo from url: "+url+" and branch: "+branch);
						return gi;
					}
				}
 			}
		}
		logger.info("Did not find application for repo from url : "+gitUrl+" branch: "+branch);
		return null;
	}
...
//		Reader fis = new FileReader("/Users/frank/git/navajo/tipi_dev/com.dexels.navajo.tipi.dev.store/json_test/single_tipi_edit.json");
		...
		ghs.setRepositoryManager(new RepositoryManager() {
			...
			public File getSshFolder() {
				return new File("/Users/frank/git/navajo/tipi_dev/com.dexels.navajo.tipi.dev.store/gitssh");
			}
