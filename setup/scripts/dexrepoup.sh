...
STATUS=`curl  -s -w "%{http_code}" "https://repo.dexels.com/repository/dexelsrepo/$urlGroupId/$artifactId/$version/$artifactId-$version.jar" -o /dev/null`
if [ $STATUS -eq "200" ]
then
...
mvn deploy:deploy-file -DrepositoryId=dexels -Durl=https://repo.dexels.com/repository/dexelsrepo -DgroupId=$groupId -DartifactId=$artifactId -Dversion=$version -Dfile=$1 -Dpackaging=jar
